...
const verifyApiKeyFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    try {
      ...
      if (response.status !== 200 || data.result === 'error') {
      ...
      if (data.result === 'success') {
        ...
        console.error('API Key Verification Failed with unknown response:', errorType);
        return { isValid: false, error: `Invalid API Key. Reason: ${errorType}` };
      }

    } catch (error: any) {
      console.error('Failed to verify API key. Full error details:', error);
      return { isValid: false, error: 'Could not connect to the verification service. Check container logs for details.' };
    }
  }
);
